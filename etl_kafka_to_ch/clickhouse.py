...
def init_clickhouse() -> None:
    ...
    result = client.command(f"CREATE DATABASE IF NOT EXISTS {settings.ch_database} ON CLUSTER company_cluster")  # pyright: ignore[reportUnknownMemberType]
    logger.info(f"Создали базу данных  '{settings.ch_database}'. Результат: {result}")

    result = client.command(  # pyright: ignore[reportUnknownMemberType]
