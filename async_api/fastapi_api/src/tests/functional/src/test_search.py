...
async def test_search_films(
    ...
    async with aiohttp_client.get(
        f'http://{test_settings.service_host}'
        f':{test_settings.service_port}/api/v1/films/search/?query={params["query"]}',
    ) as resp:
...
async def test_search_films_cache(params, aiohttp_client, es_write_data, redis_client, event_loop):
    ...
    cache_id = '_'.join([
        'movies', 'film_search', str(params['query']), str(
            params['page_size'] or 50,
        ), str(params['page_number'] or 0),
    ])
