...
class AccessTokenService(TokenService):
    def __init__(self) -> None:
        super().__init__()
        self.access_token_min = ACCESS_TOKEN_MIN

    def generate_token(self, iss: str, sub: str) -> tuple[str, int]:
...
class RefreshTokenService(TokenService):
    def __init__(self) -> None:
        super().__init__()
        self.refresh_token_weeks = REFRESH_TOKEN_WEEKS

    def generate_token(self, iss: str, sub: str, access_token: str) -> tuple[str, int]:
        header = json.dumps({"alg": "HS256", "typ": "JWT"})
        header_b64 = urlsafe_b64encode(header.encode("utf-8")).decode("utf-8").rstrip("=")

        iat = int(time.time())
        exp = iat + self.refresh_token_weeks * 7 * 24 * 60 * 60
        payload = json.dumps({"iss": iss, "sub": sub, "iat": iat, "exp": exp, "access_token": access_token})
        payload_b64 = urlsafe_b64encode(payload.encode("utf-8")).decode("utf-8").rstrip("=")
